...
# Test
## Task C
test_data = pd.read_csv('data/csv_files/final_test_C.csv', encoding = 'utf-8').iloc[:, 1:]
...
test_RelCText_tok = [tokenize(c) for c in test_RelCText]
...
test_RelCText_stop = [[w for w in q if w not in stop_words] for q in test_RelCText_tok]
...
test_relC = doc2matrix(test_RelCText_stop, model)
...
taskC_qc = [np.vstack((tr_orgQ[i], tr_relC[i])) for i in range(train_data.shape[0])] # concatenate queries and comments

taskC_qc_padding = []
for i in taskC_qc:
    if len(i) < max_len_setting:
        taskC_qc_padding.append(np.vstack((i, np.zeros([max_len_setting - len(i), 256]))))
    else:
        taskC_qc_padding.append(i[:max_len_setting, :])


val_taskC_qc = [np.vstack((val_orgQ[i], val_relC[i])) for i in range(val_data.shape[0])] # concatenate queries and comments

val_taskC_qc_padding = []
for i in val_taskC_qc:
    if len(i) < max_len_setting:
        val_taskC_qc_padding.append(np.vstack((i, np.zeros([max_len_setting - len(i), 256]))))
    else:
        val_taskC_qc_padding.append(i[:max_len_setting, :])


test_taskC_qc = [np.vstack((test_orgQ[i], test_relC[i])) for i in range(test_data.shape[0])] # concatenate queries and comments

test_taskC_qc_padding = []
for i in test_taskC_qc:
    if len(i) < max_len_setting:
        test_taskC_qc_padding.append(np.vstack((i, np.zeros([max_len_setting - len(i), 256]))))
    else:
        test_taskC_qc_padding.append(i[:max_len_setting, :])
        
        
# 5. Inputs
trainset = [(taskC_qc_padding[i], qc_label[i]) for i in range(len(taskC_qc_padding))]
validset = [(val_taskC_qc_padding[i], val_qc_label[i]) for i in range(len(val_taskC_qc_padding))]
...
t.save(net.state_dict(), 'torch_savings/cnn_taskC.pt')


# 8. Testing
x_test_reshape = np.array(test_taskC_qc_padding).reshape([-1, 1, max_len_setting, 256])
...
if Use_gpu:
    ...
    cnn_model = net
    cnn_model.eval()
    cnn_model.load_state_dict(t.load('torch_savings/cnn_taskC.pt'))
    cnn_model.to(device)
    cnn_model.eval()
else:
    device = t.device("cpu")
    cnn_model = net
    cnn_model.eval()
    cnn_model.load_state_dict(t.load(t.load('torch_savings/cnn_taskC.pt')))
    cnn_model.eval()
...
def main():
    ...
    output = pd.concat((test_data['ORGQ_ID'], test_data['RELC_ID'], ranks, 
                        pd.Series(test_similarities), pd.Series(test_pred)), 1) 
    
    save_txt('torch_savings/taskC_v2/output/taskC_cnn_output_v2.txt', output)
