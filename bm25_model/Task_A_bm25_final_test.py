...
final_test_queries = np.unique(final_test['RelQBody'])
...
hash_vocabulary = np.load('savings/taskA/hash_vocabulary.npy')
...
# np.save('savings/taskA/final_test_q_vecs.npy', final_test_q_vecs)
# np.save('savings/taskA/final_test_c_vecs.npy', final_test_c_vecs)
...
# (f) Term frequency
final_test_q_vecs = np.load('savings/taskA/final_test_q_vecs.npy') # size = (880, 8465)
final_test_c_vecs = np.load('savings/taskA/final_test_c_vecs.npy') # size = (8800, 8465)
...
def sim_q_djs(qs, rel_qs, K1, b):
    ...
    for idx in range(len(qs)):
        loc = np.where(final_test['RelQBody'] == final_test_queries[idx])[0]
...
best_K1 = np.load('savings/taskA/best_params.npy')[0]
best_b = np.load('savings/taskA/best_params.npy')[1]
threshold = np.load('savings/taskA/best_params.npy')[2]
...
def main():
    ...
    output = pd.concat((final_test['RELQ_ID'], final_test['RELC_ID'], ranks, score, label), 1) 
      
    # Save as .txt
    save_txt('savings/taskA/output/taskA_BM25_output.txt', output)
