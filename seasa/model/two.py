...
class Encoder(nn.Module):
    ...
    def forward(self,query,query_len,answer,answer_len):
        ...
        def feat_extract(query_output,query_length,answer_output,answer_length,mask):
            ...
            query_normal = query_output.div(query_length).div(self.hidden_dim.sqrt())
            answer_output = query_output.div(answer_length).div(self.hidden_dim.sqrt())
        ...
        def attention(query_output,query_length,answer_output,answer_length,mask):
            ...
            # batch*feat_len*1
            query_normal = query_output.div(self.hidden_dim.sqrt()).transpose(1,2)

            #get the attention
            #batch*sent_ans*sent_qu
            attn_weight = answer_output.bmm(query_normal)
            
            final_mask = mask['answer'].bmm(mask['query'].transpose(1,2))>0.5
